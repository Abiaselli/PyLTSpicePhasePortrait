...
SEARCH_SPACE = {
    ...
    "I_activate": [0.20e-6, 0.50e-6, 1.00e-6],
    "I_deactivate": [-0.20e-6, -0.50e-6],
    "vdd": [3.3],
...
def add_optional_bias_overrides(text: str, cand: Candidate) -> str:
    ...
    text2 = text
    for ref, value in (("Vdd", cand.vdd), ("VDD", cand.vdd), ("Vc", cand.vc), ("VTH", cand.vth), ("Vth", cand.vth), ("Vbias", cand.vbias), ("VBIAS", cand.vbias)):
...
def build_phase_netlist(base_text: str, cand: Candidate) -> str:
    ...
    extra = [
        ...
        *append_topology_lines(cand, mode="phase"),
        f".step param UU {PHASE_U_RANGE[0]} {PHASE_U_RANGE[1]} {PHASE_U_RANGE[2]}",
        f".dc V_VSTATE {PHASE_V_RANGE[0]} {PHASE_V_RANGE[1]} {PHASE_V_RANGE[2]}",
        ".op",
...
def analyze_transient(raw_path: Path, plot_prefix: Path) -> dict:
    ...
    # On LTspice transient RAW files the first plot should be the transient plot.
    # Prefer direct trace lookup on that plot; fall back to the explicit plot picker.
    ...
    # Prefer get_axis() for transient data; if it fails, fall back to the explicit 'time' trace.
    try:
    ...
    plt.axvspan(0.010, 0.020, alpha=0.15)
    plt.axvspan(0.060, 0.070, alpha=0.15)
...
def run_search(limit: int | None = None) -> list[RunResult]:
    ...
    for idx, cand in enumerate(candidate_iterator(), start=1):
        ...
        name = sanitize_name(
            f"run_{idx:04d}_{cand.topology}_Cv_{eng(cand.Cv)}_Cu_{eng(cand.Cu)}_Ib_{eng(cand.I_bias)}_Ia_{eng(cand.I_activate)}_Id_{eng(cand.I_deactivate)}"
        )
        ...
        print(f"[{idx}] {name}: total_score={result.total_score:.2f} coast_spikes={result.coast_spikes} final_spikes={result.final_spikes}")
...
def write_results_csv(results: Sequence[RunResult], csv_path: Path) -> None:
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        w.writerow([
            ...
            "vdd_V", "vc_V", "vth_V", "vbias_V",
            "phase_ok", "transient_ok", "fixed_point_count", "stable_point_count",
            "coast_spikes", "final_spikes", "phase_score", "bistability_score", "total_score",
        ...
        for rank, r in enumerate(results, start=1):
            ...
            w.writerow([
                ...
                c.vdd, c.vc, c.vth, c.vbias,
                int(r.phase_ok), int(r.transient_ok), r.fixed_point_count, r.stable_point_count,
                r.coast_spikes, r.final_spikes, r.phase_score, r.bistability_score, r.total_score,
...
if __name__ == "__main__":
    ...
    for r in results[:5]:
        print(
            ...
            f"coast_spikes={r.coast_spikes}, final_spikes={r.final_spikes}, notes={r.notes}"
        )
